...
$jumlahBerita = count(query("SELECT * FROM postingan WHERE info_kategori = '$idKategori' ORDER BY id DESC"));
...
$postingan = query("SELECT * FROM postingan WHERE info_kategori = '$idKategori' ORDER BY id DESC");
...
<body style="background-color: gainsboro;">
    ...
    <div class="pagination">
        <?php if($halamanAktif != 1){ 
            $a = $halamanAktif -1;
            echo "<a class='page-link' href='?halaman=$a&id=$idKategori'>Previous</a>";
        } ?>
        <!-- <a class="page-link" href="?halaman=<?= $jumlahHalaman-1; ?>">Previous</a> -->
        <?php for( $i = 1; $i <= $jumlahHalaman; $i++ ) : 
// var_dump($i);
            ?>

            <?php if($halamanAktif != $i) : ?>
                <a class="page-link" href="?halaman=<?= $i;?>&id=<?=$idKategori;?>"><?= $i; ?></a>
            <?php else : ?>
        ...
        <?php if($halamanAktif < $jumlahHalaman){ 
            $a = $halamanAktif +1;
            echo "<a class='page-link' href='?halaman=$a&id=$idKategori'>Next</a>";
        } ?>
    </div>
        <footer>
            @copyright 2021
